...
def process_java_folder_mirrored(folder_path, source_root, test_root):
    ...
    rel_parts = rel_folder.split(os.sep) if rel_folder != '.' else []
    # The test folder path mirrors the source, with 'Test' appended to each part
    test_folder_parts = [os.path.basename(source_abs) + 'Test'] if not rel_parts else [os.path.basename(source_abs) + 'Test'] + [part + 'Test' for part in rel_parts]
    test_folder = os.path.join(parent_dir, *test_folder_parts)
    os.makedirs(test_folder, exist_ok=True)
    source_codes = []
    file_names = []
    for fname in java_files:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(source_root):
        ...
        print(f"[INFO] Generating test folders as siblings to source folders, with 'Test' appended to each folder name.")
        dfs_process_java_folders_mirrored(source_root, test_root=None)
